...
export function useAgentExitListener(deps: UseAgentExitListenerDeps): void {
	useEffect(() => {
		...
		const unsubscribe = window.maestro.process.onExit(
			async (sessionId: string, code: number) => {
				...
				if (isFromAi) {
					...
					if (currentSession) {
						...
						toastData = {
							...
							startTime:
								completedTabData?.thinkingStartTime || currentSession.thinkingStartTime,
							isRemote: !!(
				...
				if (queuedItemToProcess) {
					...
					setTimeout(() => {
						...
						const currentActiveSession = getSessions().find(
							(s) => s.id === getActiveSessionId()
						);
